...
function sourceOf(f: Facts): string {
  ...
  return [
    ...
    // Recorded Eternal loadouts (pred.gg perks). A film without them is NOT a
    // licence to name a player's Eternal: the author may then speak only of the
    // engine's pick, labelled as such (maintainer feedback 2026-09-21).
    `LOADOUTS RUN (from the match feed; the ONLY source for what a player equipped): ${us.map((p) => `${p.squadName || p.name} ${p.heroName}: ${(p as any).loadout?.eternal ? `${(p as any).loadout.eternal}${((p as any).loadout.minors ?? []).length ? ' + ' + (p as any).loadout.minors.join(' + ') : ''}${(p as any).loadout.augment ? `, augment ${(p as any).loadout.augment}` : ''}` : 'NOT RECORDED — any Eternal named for this player must be introduced as the engine\'s pick, never as what they ran'}`).join('; ')}.`,
...
(h) LOADOUT — an Eternal/blessing line that states or implies a player RAN a given Eternal, minor or augment ("X's loadout is Vesh", "X ran Thraex", "X's top pick", "the loadout he took", or judging how "the loadout played out" as if equipped) when the SOURCE's LOADOUTS RUN row for that player says NOT RECORDED — or names a different Eternal/minor than the one recorded there. Rewrite so the pick is introduced as "The engine's pick for X is …" (when not recorded) or corrected to the recorded loadout; keep the fit-and-trade-off reasoning and every number.
...
GAME KNOWLEDGE IS NOT A MATCH CLAIM: plain-word item/Eternal mechanics and map-economy concepts (objective spawn windows and cadence, "power farm"/"invade"/"deward", build-swap conditions like "against heavy healing") need NO SOURCE support — do not flag them as ungrounded. Any NUMBER, and any claim about what happened in THIS game, still requires the SOURCE.
