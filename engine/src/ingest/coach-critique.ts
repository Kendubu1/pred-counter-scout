...
function sourceOf(f: Facts): string {
  ...
  return [
    ...
    `OUR PLAYERS: ${us.map((p) => `${p.squadName || p.name} ${p.heroName} ${p.role} ${p.kills}/${p.deaths}/${p.assists}, ${(p as any).damageToHeroes ?? '?'} hero dmg, ${(p as any).damageToObjectives ?? '?'} obj dmg, ${(p as any).healingDone ?? '?'} healing, ${(p as any).mitigated ?? '?'} mitigated, ${(p as any).wardsPlaced ?? '?'} wards, items [${((p as any).items ?? []).map((i: any) => i.name ?? i).join(', ')}]${((p as any).itemTimeline ?? []).length ? `, est online [${(p as any).itemTimeline.map((t: any) => `${t.name}~${t.estMin}m`).join(', ')}]` : ''}${((p as any).matchupItemFlags ?? []).length ? `, film flags [${(p as any).matchupItemFlags.join('; ')}]` : ''}${((p as any).missingCore ?? []).length ? `, missing meta core [${(p as any).missingCore.join(', ')}]` : ''}${(p.spikes ?? []).length ? ` (spikes ${p.spikes.map((s: any) => `${s.name}~${s.spikeMinute}m`).join(', ')})` : ''}${p.roleFit?.concern ? ` [off bottom-two lane]` : ''}`).join('; ')}.`,
