...
type Verdict = 'applied' | 'stale' | 'unknown' | 'pending' | 'frozen';
interface Check {
...
function main() {
  ...
  // Upstream frozen: omeda.city stopped publishing on 2026-08-28 (payload
  // byte-identical since, see lessons.md 2026-09-21). A patch released AFTER
  // the last content change can never land in that catalog, so its stale
  // checks are a different state from "we didn't look": reclassified 'frozen'
  // ONLY when the snapshot's content has not moved for FROZEN_MIN_DAYS (proof
  // the upstream is dead, not merely late), the patch was released after the
  // last content change, and we are still re-fetching. It stays loud in the
  // verdict, is never counted as applied, and the engine is knowingly on
  // pre-patch numbers — the committed patch digests are the record of what
  // changed. It is the alarm, made truthful instead of permanent.
  const FROZEN_MIN_DAYS = 14;
  const meta = JSON.parse(readFileSync(path.join(ROOT, 'data/omeda/META.json'), 'utf8')) as { contentChangedAt?: string; contentAgeDays?: number };
  const frozenPatches: string[] = [];
  const frozenReleaseDates: Record<string, string> = {};
  if (meta.contentChangedAt && (meta.contentAgeDays ?? 0) >= FROZEN_MIN_DAYS) {
    const frozenAt = Date.parse(meta.contentChangedAt);
    for (const file of files) {
      const d = JSON.parse(readFileSync(path.join(ROOT, 'data/patches', file), 'utf8')) as { patch?: string; date?: string };
      const patch = d.patch ?? file.replace('.json', '');
      if (!d.date || Date.parse(d.date) <= frozenAt) continue;
      const st = checks.filter((c) => c.patch === patch && (c.verdict === 'stale' || c.verdict === 'pending'));
      if (!st.length) continue;
      frozenPatches.push(patch); frozenReleaseDates[patch] = d.date;
      for (const c of st) c.verdict = 'frozen';
    }
    if (pendingPatch && frozenPatches.includes(pendingPatch)) pendingPatch = null;
  }
  ...
  for (const [patch, cs] of byPatch) {
    ...
    const fz = cs.filter((c) => c.verdict === 'frozen').length;
    const flag = s > 0 ? '  <-- STALE VALUES PRESENT' : p > 0 ? '  <-- PENDING (omeda has not published this patch yet)' : fz > 0 ? '  <-- FROZEN (released after omeda stopped publishing)' : '';
    console.log(`${patch.padEnd(8)} applied ${String(a).padStart(3)} · stale ${String(s).padStart(3)} · pending ${String(p).padStart(3)} · frozen ${String(fz).padStart(3)} · unknown ${String(u).padStart(3)}${flag}`);
  ...
  const frozen = checks.filter((c) => c.verdict === 'frozen').length;
  ...
  else if (frozenPatches.length) console.log(`VERDICT: UPSTREAM FROZEN — omeda's catalog has not changed since ${meta.contentChangedAt!.slice(0, 10)} (${meta.contentAgeDays} days); ${frozenPatches.join(', ')} released after that and can never land in it (${frozen} stated changes). The engine is knowingly on pre-${frozenPatches[0]} numbers; the committed patch digests are the record of those changes. Keep re-fetching daily so a revived upstream is caught.`);
  ...
  const out = {
    ...
    snapshotFetchedAt,
    totals: { checks: total, applied, stale, pending, frozen, unknown: total - applied - stale - pending - frozen, unparsedNoteLines: unparsed },
    staleAgainstPatch: newestStale,
    frozenAgainstPatches: frozenPatches,
    frozenReleaseDates,
    upstreamContentChangedAt: meta.contentChangedAt ?? null,
    upstreamContentAgeDays: meta.contentAgeDays ?? null,
    ...
    perPatch: Object.fromEntries([...byPatch].map(([p, cs]) => [p, {
      ...
      frozen: cs.filter((c) => c.verdict === 'frozen').length,
