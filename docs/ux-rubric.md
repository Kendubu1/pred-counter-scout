# Predecessor Scout — pinned Senior-UX rubric

This is the **fixed target** the author and the independent judge both read. It does
not move under the generator. It is the "definition of done" for the UX-review
loop (see `docs/agent-loops.md` → "UX loop").
...
run with `UI_DIR=<dir>`) or `[JUDGE]` (scored by `pred-scout-ux-judge` from the
...
| **R7** | **Cross-page consistency** (constraint) | AUDIT (existing) | Reuse the v6 invariants — shared `:root` tokens, one container width, one breakpoint, base-reset parity, viewport meta, inline `node --check` — run against the shipped pages. |

## The brackets (objective; a round only counts when these are green)

- `npm run ui:audit` — exit 0 = all hard invariants pass (the 11 v6 families + the
  three long-form ones: `single-legend`, `reduced-motion`, `above-fold-primary`).
- `npm run ui:render` — exit 0 = no horizontal overflow on any phone surface
  (360/390px); writes screenshots to `docs/reviews/v6/shots/`.
