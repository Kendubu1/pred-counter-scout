...
- Same day, after merging main's "omeda stopped publishing" finding: the
  icon sync no longer consults omeda at all. pred.gg's catalog carries an
  `icon` hash for heroes (54/54) and items (270 non-hidden of 282), so hero,
  item and crest icons now come from the same source as the perks. Ability
  icons stay committed under their omeda hashes (the frozen kit snapshot is
